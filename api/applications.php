...
function submitApplication() {
    ...
    // Check if job exists and is active - also verify employer exists
    $jobStmt = $db->prepare("SELECT j.employer_id, j.status, e.id as employer_exists 
                             FROM jobs j 
    ...
    // Check if employer still exists
    if ($job['employer_exists'] === null) {
        jsonResponse(['success' => false, 'message' => 'This job posting is no longer available. The employer account has been removed.'], 400);
    }
    ...
    $employerId = intval($job['employer_id']);
